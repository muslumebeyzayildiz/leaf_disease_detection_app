...
class galeriState extends State<galeri> {
  ...
  void initState() {
    ...
    loadModel().then((value) {
      //TensorFlow Lite modeli yükleniyor ve model yüklendiğinde
    setState(() {
  ...
  loadModel() async {
    await Tflite.loadModel(
      ...
      // Bu yol,  projenizin içinde yer alan ve TensorFlow Lite modelinin bulunduğu dosyayı göstermelidir. Dosya uzantısı .tflite olmalıdır.
  ...
  sinifliResim(File image) async {
    ...
    if (_outputs != null && _outputs!.isNotEmpty) {
      //tespit sonuçlarının mevcut ve geçerli olduğunda, yani tespit yapıldığında,
      // tespit sonuçlarını Firestore'a kaydetmek için kullanılır.
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: _loading
          ? Container(  // loading true ise İşlem sürerken yükleme animasyonu gösterilir
              alignment: Alignment.center,
              child: CircularProgressIndicator(),
               ) // Container
          : SingleChildScrollView(//kaydırma
        //_loading false sayüklenme işlemi tamamlandı demektir.
    child: Container(
        width: MediaQuery.of(context).size.width,//todo cihaz ekranının genişliğini temsil eder.
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.center,
          //çocuk widget'ların yatay (x ekseninde) hizalamasını belirler. '
          //  'CrossAxisAlignment.center kullanıldığında, çocuk widget'lar yatayda
          //  (sol-dan ve sağdan) ekranda merkeze hizalanır.
          mainAxisAlignment: MainAxisAlignment.center,
          //çocuk widget'lar dikeyde (üstten ve alttan) ekranda merkeze hizalanır.
          children: [
            _image == null
                ? Container() // Eğer kullanıcı henüz bir resim seçmemişse (_image null ise), bu durumda resmi göstermek için bir alanın olmaması gerektiğinden, boş bir Container gösterilir.
                : Container(
              child: Image.file(_image!),// Seçilen resim gösterilir
              height: 500,
              width: MediaQuery.of(context).size.width - 200,//todo cihaz ekranının genişliğini temsil eder.
            ), // Container
            const SizedBox(
              height: 20,
            ), // SizedBox

            _outputs!= null && _outputs!.isNotEmpty //todo _outputs!= null böyleydi && _outputs!.isNotEmpty eklenince
            ...
            // Eğer _outputs değişkeni null ise veya içeriği boşsa, sınıflandırma sonuçlarının gösterilmesi yerine "Sınıflandırma bekleniyor" mesajının gösterilmesini sağlar.
                ...
                ? Column(//Sınıflandırma sonuçlarının mevcut ve geçerli olduğunda
                   children: [
                  Text(//Tespit Edilen Hastalık:
                  " ${_outputs?[0]["label"]}",
                     // .replaceAll(RegExp(r' [0-9]'), ''),//1 healthy değilde healthy desin // bir şey değişmiyordu ama
                  style: TextStyle(
                    color: Colors.black,
                    fontSize: 20.0,
                ...
                : Column(//Sınıflandırma sonuçlarının mevcut ve geçerli OLMAMASI
                  children: [
                    _showAnimation // _showAnimation durumuna göre animasyonu gösteriyoruz

                    ? Lottie.network(//_showAnimation değişkeni true olduğu sürece, bu widget animasyonu gösterir.
                    'https://lottie.host/aa670ff0-1820-4708-b5df-873cec8dc9eb/1CsXTp2leL.json', // Lottie animasyonunun URL'si
                   width: 400,
                    height: 400,
                         )
                    : Container(),//yanlışsa boş Container widget'ı kullanılarak hiçbir şey görüntülenmez. Yani, animasyon gösterilmez.
